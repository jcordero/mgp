 ...
 *
 */
class class_tic_ticket_upd_sol_hooks extends cclass_maint_hooks
{
    ...
    //	Si NOTIFICADA -> TRANSFERIDA,CUMPLIDA,RECHAZADA
    //	Si TRANSFERIDA -> CUMPLIDA,RECHAZADA
    //	Si CUMPLIDA -> Nada
    //	Si RECHAZADA -> Nada
    ...
    public function beforeSaveDB()
	{
        ...
        //Es una denuncia?
        if($this->m_tipo!="SOLICITUD")
        {
        	$res[] = "MENSAJE: Solo puede aplicar esta operación a los tickets de SOLICITUD";
        	return $res;
        }
        ...
        if( $row=$primary_db->_fetch_array($re) )
        {
        	if($row["cant"]==0)
        	{
        		$res[] = "MENSAJE: Su perfil de usuario no permite modificar a esta solicitud";
        		return $res;
        	}
        }
        $primary_db->_free_result($re);
        ...
        while( $row=$primary_db->_fetch_array($re,$j++) )
        {
        	...
        	if($estado=="INICIADO")
        	{
        		if($nuevo_estado!="INICIADO" && $nuevo_estado!="NOTIFICADA" && $nuevo_estado!="")
        		{
        			$res[] = "MENSAJE: La solicitud, prestacion $prest solo puede cambiar a estado NOTIFICADA";
        			return $res;
        		}	
        	}
        	elseif($estado=="NOTIFICADA")
        	{
	        	if($nuevo_estado!="NOTIFICADA" && $nuevo_estado!="TRANSFERIDA"  && $nuevo_estado!="CUMPLIDA" && $nuevo_estado!="RECHAZADA" && $nuevo_estado!="")
        		{
        			$res[] = "MENSAJE: La solicitud, prestacion $prest solo puede cambiar a estado NOTIFICADA,TRANSFERIDA,CUMPLIDA o RECHAZADA.";
        			return $res;
        		}
        	}
        	elseif($estado=="TRANSFERIDA")
        	{
	        	if($nuevo_estado!="TRANSFERIDA" &&  $nuevo_estado!="CUMPLIDA" && $nuevo_estado!="RECHAZADA" && $nuevo_estado!="")
        		{
        			$res[] = "MENSAJE: La solicitud, prestacion $prest solo puede cambiar a estado TRANSFERIDA,CUMPLIDA o RECHAZADA.";
        			return $res;
        		}
        	}
        	elseif($estado=="CUMPLIDA")
        	{
        		...
        		{
        			$res[] = "MENSAJE: La solicitud, prestacion $prest no se puede cambiar de estado. Esta CUMPLIDA.";
        			return $res;
        		}	
        	}
        	elseif($estado=="RECHAZADA")
        	{
	        	if($nuevo_estado!="RECHAZADA" && $nuevo_estado!="")
        		{
        			$res[] = "MENSAJE: La solicitud, prestacion $prest no se puede cambiar de estado, Esta RECHAZADA.";
        			return $res;
        		}
        	}
        	else
        	{
        		$res[] = "MENSAJE: La solicitud,prestacion $prest esta en estado '$estado' que no es reconocido por el sistema.";
        		return $res;
        	}	
			...
			if($nuevo_estado=="TRANSFERIDA")
			{
				...
				else
				{
					...
					$res[] = "MENSAJE: La solictud, prestacion $prest se ha solicitado transferencia pero no se ha indicado el organismo destino.";
        			return $res;
				}
			...
			if($estado!=$nuevo_estado && $nuevo_estado!="")
			{
			    ...
			    if($nuevo_estado=="CUMPLIDA" || $nuevo_estado=="RECHAZADA")
			    {
