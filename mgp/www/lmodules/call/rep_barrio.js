...
function graficar()
{
	...
	var url = sess_web_path+ "/common/rem_request.php?presentation=REPORTES&func=barrio&args="+desde+"|"+hasta;
	$.getJSON(url, function(data) {		
		$("#tmp_graph .fld").html("");
		if(data.result=="OK")
		$("#tmp_graph .fld").append("<div class='graficos'>"+data.image.resultado + data.image.actitud + data.image.barrio + "</div>");
	});	
}
...
function reportar()
{
	...
	//Llamo al presentation REPORTES
	var url = sess_web_path+"/common/rem_request.php?presentation=REPORTES&func=reporteBarrio&args="+desde+"|"+hasta;
	$.getJSON(url, function(data) {		
		p.html("");
		if(data.result=="OK")
		{
