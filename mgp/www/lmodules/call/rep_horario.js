...
function graficar()
{
	...
	var url = sess_web_path+"/common/rem_request.php?presentation=REPORTES&func=horarios&args="+desde+"|"+hasta;
	$.getJSON(url, function(data) {		
		$("#tmp_graph .fld").html("");
		if(data.result=="OK")
		$("#tmp_graph .fld").append("<div class='graficos'>"+data.image.resultado + data.image.actitud + data.image.horarios + "</div>");
	});	
}
...
function reportar()
{
	...
	//Llamo al presentation REPORTES
	var url = sess_web_path+"/common/rem_request.php?presentation=REPORTES&func=reporteHorarios&args="+desde+"|"+hasta;
	$.getJSON(url, function(data) {		
		p.html("");
		if(data.result=="OK")
		{
