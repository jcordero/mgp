        ...
        if ($this->tic_identificador == '' && ($this->tic_nro == '' || $this->tic_nro == 0)) {
            ...
            if ($this->tic_nro == 0 || $this->tic_nro == '') {
                ...
                if (intval($this->tic_nro) == 0) {
                    //Pruebo si es una denuncia... porque el identificador por defecto RECLAMO ... no trae resultado
                    $p = explode(" ", $this->tic_identificador);
                    $this->tic_identificador = "DENUNCIA " . $p[1];
                    $this->tic_nro = $primary_db->QueryString("select tic_nro from tic_ticket where tic_identificador='{$this->tic_identificador}'");
                    if (intval($this->tic_nro) == 0) {
                        //Pruebo si es una solicitud... porque el identificador por defecto RECLAMO o DENUNCIA ... no trae resultado
                        $this->tic_identificador = "SOLICITUD " . $p[1];
                        $this->tic_nro = $primary_db->QueryString("select tic_nro from tic_ticket where tic_identificador='{$this->tic_identificador}'");
                        if (intval($this->tic_nro) == 0) {
