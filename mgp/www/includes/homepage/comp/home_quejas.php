...
if(!class_exists('home_quejas'))
{
	...
	class home_quejas
	{
		public function Render($context)
		{
            ...
            $html.= '<div id="home_quejas" class="apTabs">';
...
//Ingresos
            ...
            foreach($dias as $dia => $cant)
            {
           	...
           	if($k==0)
           	{
           		$xml = "<?xml version='1.0' encoding='UTF-8' ?><graph showNames='1' decimalPrecision='0' yaxismaxvalue='10' caption='Ingresos de quejas (últimos 30 días)'>".$xml;
           	}
           	else
           	{
           		$xml = "<?xml version='1.0' encoding='UTF-8' ?><graph showNames='1' decimalPrecision='0' caption='Ingresos de quejas (últimos 30 días)'>".$xml;
           	}
...
//Cierres
            ...
            foreach($dias as $dia => $cant)
            {
           	...
           	if($k==0)
           	{
	           	$xml = "<?xml version='1.0' encoding='UTF-8' ?><graph showNames='1' decimalPrecision='0' yaxismaxvalue='10' caption='Cierres de quejas (últimos 30 días)'>".$xml;
           	}
           	else
           	{
           		$xml = "<?xml version='1.0' encoding='UTF-8' ?><graph showNames='1' decimalPrecision='0' caption='Cierres de quejas (últimos 30 días)'>".$xml;
           	}
...
//Distribucion
		...
		private function getOrganismos()
		{
			...
			if( isset($_SESSION['groups']) )
        	{
     		...
     		{
     			error_log("HOME QUEJAS: No esta definida la sesion");
     		}
        	return $sigla;
		}
		...
		{
			global $primary_db;
			$sql = "SELECT count(*) as cant,DAYOFYEAR(ttp_tstamp) as dia FROM v_tickets WHERE tor_code in ($lista) AND ttp_estado not in ('CUMPLIDA','RECHAZADA') and DAYOFYEAR(ttp_tstamp) between (DAYOFYEAR(NOW())-30) and (DAYOFYEAR(NOW())) GROUP BY DAYOFYEAR(ttp_tstamp) order by 2";
			$conjunto = array();
		...
		{
			global $primary_db;
			$sql = "SELECT count(*) as cant,DAYOFYEAR(ttp_tstamp) as dia FROM v_tickets WHERE tor_code in ($lista) AND ttp_estado in ('CUMPLIDA','RECHAZADA') and DAYOFYEAR(ttp_tstamp) between (DAYOFYEAR(NOW())-30) and (DAYOFYEAR(NOW())) GROUP BY DAYOFYEAR(ttp_tstamp) order by 2";
			$conjunto = array();
			$rs = $primary_db->do_execute($sql);
			if($rs) 
			{
