...
class CDH_CUESTIONARIO2 extends CDataHandler
{
    ...
    function loadForm()
	{
        ...
        $cuestionario = $_REQUEST["m_".$fld->m_ClassParams];
        if($cuestionario=="")
        {
            $fld->setValue("");
            return;
        ...
        while( $myrow=$primary_db->_fetch_row($re) )
        {
            ...
            $result.= '<tipo>'.$myrow['spr_presentacion'].'</tipo>';
            $result.= '<pregunta>'.$myrow['spr_pregunta'].'</pregunta>';
